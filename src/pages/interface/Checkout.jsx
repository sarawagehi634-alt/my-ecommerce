...
import { 
  ...
  FaMoneyBillWave, FaUniversity, FaUpload, FaCheckCircle, FaTimesCircle
} from 'react-icons/fa';
...
const Checkout = () => {
  ...
  const saveOrderLocally = () => {
    ...
    const receiptBase64 = imagePreview || null;
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.paymentMethod === 'bank_transfer' && !transferImage)
      return toast.error('الرجاء رفع صورة إيصال التحويل البنكي');

    setLoading(true);
    setUploadProgress(0);

    const interval = setInterval(() => {
      setUploadProgress(prev => (prev >= 90 ? (clearInterval(interval), 90) : prev + 10));
    }, 200);
    ...
    try {
      ...
      try {
        ...
        toast.success(formData.paymentMethod === 'bank_transfer' ? 'تم استلام طلبك وإيصال التحويل، سيتم المراجعة' : 'تم إنشاء الطلب بنجاح');
        clearCart();
        setTimeout(() => navigate(`/order-success/${response.data.id || response.data.order_id || 'new'}`), 1000);
        ...
        if (mainError.response?.status === 404) {
          ...
          if (!fallbackSuccess) {
            ...
            clearInterval(interval);
            setUploadProgress(100);
            toast.success('تم حفظ طلبك بنجاح (وضع التطوير)', { icon: '💾', duration: 5000 });
            clearCart();
      ...
      <div className="container-custom">
        ...
        {cartItems.length === 0 ? (
          ...
          <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-8" noValidate>
            {/* …باقي النموذج والملخص كما في النسخة الأصلية لكن منسقة وأقصر للتوضيح… */}
          </form>
        )}
      </div>
    </div>
