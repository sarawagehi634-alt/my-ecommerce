  ...
  return (
    <div className="min-h-screen py-12 bg-gray-50" dir="rtl">
      <div className="container-custom max-w-5xl">
        <h1 className="text-4xl font-bold text-gray-900 mb-12 text-center">سياسة الشحن والتوصيل</h1>
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 space-y-12">
          ...
          <section>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {shippingPolicy?.methods?.map((method, index) => (
                <div
                  key={index}
                  className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm cursor-pointer transition-transform transform hover:scale-105 hover:shadow-lg"
                  title={`اضغط لعرض تفاصيل ${method.name}`}
                >
                  <h3 className="font-semibold text-lg mb-2 text-gray-800">{method.name}</h3>
          ...
          <section>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="bg-gray-50 p-6 rounded-2xl shadow-sm">
                <h3 className="font-semibold mb-3 text-gray-800">المدن الرئيسية</h3>
                <div className="flex flex-wrap gap-2">
                  {shippingPolicy?.cities?.map((city, index) => (
                    <span key={index} className="bg-primary-100 text-primary-700 px-3 py-1 rounded-full text-sm">{city}</span>
                  ))}
                </div>
              </div>
              <div className="bg-gray-50 p-6 rounded-2xl shadow-sm">
                <h3 className="font-semibold mb-3 text-gray-800">شحن لجميع المدن</h3>
          ...
          <section className="bg-primary-50 p-6 rounded-2xl shadow-sm">
            ...
            <ul className="list-disc list-inside text-gray-700 space-y-2 mr-4">
              {shippingPolicy?.notes?.map((note, index) => <li key={index}>{note}</li>)}
