...
class CategoryService {
  ...
  async getCategories(params = {}) {
    try {
      ...
      if (error.response?.status === 404) {
        return {
          status: true,
          message: '👗 لا توجد أقسام في المتجر',
  ...
  async createCategory(formData) {
    try {
      const response = await api.post('/dashboard/categories', formData, {
  ...
  async updateCategory(id, formData) {
    try {
      ...
      const response = await api.post(`/dashboard/categories/${id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      return response.data;
    } catch (error) {
  ...
  async getParentCategories() {
    try {
      ...
      if (error.response?.status === 404) {
        return { status: true, message: '👗 لا توجد أقسام رئيسية', data: [] };
      }
  ...
  async getInterfaceCategories(params = {}) {
    try {
      const response = await api.get('/interface/categories', { params });
      return response.data;
    } catch (error) {
      if (error.response?.status === 404) {
        return { status: true, message: '👗 لا توجد أقسام متاحة', data: [] };
      }
      this.handleError(error, '❌ حدث خطأ في جلب الأقسام');
    }
  }
  ...
  async getInterfaceCategory(id) {
    try {
      const response = await api.get(`/interface/categories/${id}`);
      return response.data;
    } catch (error) {
      if (error.response?.status === 404) {
        throw new Error('👗 القسم غير موجود');
  ...
  async getCategoryProducts(id, params = {}) {
    try {
      const response = await api.get(`/interface/categories/${id}/products`, { params });
      return response.data;
    } catch (error) {
